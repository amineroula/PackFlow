...
def _footer(canvas, doc):
    canvas.saveState()
    canvas.setStrokeColor(colors.HexColor("#D7DAD7"))
    canvas.setLineWidth(0.5)
    canvas.line(0.55 * inch, 0.42 * inch, 7.95 * inch, 0.42 * inch)
    canvas.setFillColor(FORMATIVE["olive"])
    canvas.setFont("Courier", 7)
    canvas.drawString(0.55 * inch, 0.25 * inch, "PACKFLOW / PACKAGING GUIDE 01")
...
def export_packaging_guide(guide: PackagingGuide, output_path: str) -> None:
    doc = SimpleDocTemplate(
        ...
        title=f"Packaging Guide — {guide.part_number or 'Part'}",
        author="PackFlow",
    )
    ...
    story = [
        Paragraph("FORMATIVE 3D", title),
        Paragraph("PACKAGING GUIDE", eyebrow),
        Spacer(1, 0.05 * inch),
        Paragraph(
            f"{guide.part_number or 'PART NUMBER'} — {guide.part_name or 'PART NAME'}",
            title,
        ),
        Spacer(1, 0.18 * inch),
    ]
    ...
    info_panel = Table(
        [
            ...
            [Paragraph("ARRANGEMENT", eyebrow), Paragraph(guide.arrangement_text, body)],
        ],
    ...
    info = Table(
        [[_safe_image(guide.part_image_path, 3.55 * inch, 2.15 * inch), info_panel]],
    ...
    story += [
        ...
        Spacer(1, 0.06 * inch),
        _safe_image(guide.packing_image_path, 6.4 * inch, 3.45 * inch),
        Spacer(1, 0.12 * inch),
        Paragraph("Place the parts in the box exactly as shown in the reference image.", body),
        PageBreak(),
        Paragraph("FORMATIVE 3D", title),
        Paragraph("PACKING STEPS", eyebrow),
        Spacer(1, 0.05 * inch),
        Paragraph(
            f"{guide.part_number or 'PART NUMBER'} — Packing Instructions",
            title,
        ),
        Spacer(1, 0.12 * inch),
    ]
    ...
    for index, step in enumerate(guide.steps[:4], start=1):
        ...
        content = Table(
            [
                [number, Paragraph(step.title or f"Step {index}", step_title)],
                ["", Paragraph(step.instruction or "", body)],
                ["", _safe_image(step.image_path, 2.55 * inch, 1.72 * inch)],
            ],
